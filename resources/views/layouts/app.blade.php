...
<nav class="w-full py-4 bg-blue-800 shadow">
    <div class="w-full container mx-auto flex flex-wrap items-center justify-center">
        <div class="flex items-center text-lg no-underline text-white pr-6">
            ...
            <a target="_blank" class="pl-6" href="{{$set['instagram']}}">
                <i class="fab fa-instagram"></i>
            </a>

            <a target="_blank" class="pl-6" href="{{$set['linkedin']}}">
...
<nav class="w-full py-4 border-t border-b bg-gray-100" x-data="{ open: false }">
    ...
    <div :class="open ? 'block': 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
        <div
            ...
            @foreach($cat as $list)
                <a href="{{route('category',[$list->id,$list->slug])}}"
                   class="hover:bg-gray-400 rounded py-2 px-4 mx-2">{{$list->category}}</a>
            @endforeach
        </div>
    </div>
</nav>
